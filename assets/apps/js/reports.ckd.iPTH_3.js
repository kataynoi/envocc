$(function () {
    ...
    reports.ajax_cross = {
        get_control_bp: function (year,url, cb) {
            var url = url+'ckd_02_02_13_stage3.php',
                params = {
                    year: year
                }
    ...
    reports.set_control_bp = function (data) {
        $('#tbl_list > tbody').append(
            '<tr>' +
                '<td></td>'+
                '<td>'+ data.province+'</td>' +
                '<td>' + app.add_commars_with_out_decimal(data.ckd_result_all) + '</td>' +
                '<td>' + app.add_commars_with_out_decimal(data.ckd_result_ok) + '</td>' +
                '<td>' + app.add_commars_with_out_decimal(data.ckd_result_not_ok) + '</td>' +
                '</tr>'
        );
