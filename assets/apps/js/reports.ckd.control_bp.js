$(function () {
    ...
    reports.ajax_cross = {
        get_control_bp: function (year,url, cb) {
            var url = url+'ckd_02_02_01_original.php',
                params = {
                    year: year
                }
    ...
    reports.set_control_bp = function (data) {
        $('#tbl_list > tbody').append(
            '<tr>' +
                ...
                '<td>' + app.add_commars_with_out_decimal(data.ckd_bp) + '</td>' +
                '<td>' + app.add_commars_with_out_decimal(data.ckd_bp_ok) + '</td>' +
                '<td>' + app.add_commars_with_out_decimal(data.ckd_bp_not_ok) + '</td>' +
                '</tr>'
        );
