$(function () {
    ...
    reports.ajax_cross = {
        get_control_bp: function (year,url, cb) {
            var url = url+'ckd_02_02_05_original.php',
                params = {
                    year: year
                }
    ...
    reports.set_control_bp = function (data) {
        $('#tbl_list > tbody').append(
            '<tr>' +
                '<td></td>'+
                '<td>'+ data.province+'</td>' +
                '<td>'+ app.add_commars_with_out_decimal(data.dm_all)+'</td>' +
                '<td>' + app.add_commars_with_out_decimal(data.dm_hba1c) + '</td>' +
                '<td>' + app.add_commars_with_out_decimal(data.dm_hba1c_ok) + '</td>' +
                '<td>' + app.add_commars_with_out_decimal(data.dm_hba1c_not_ok) + '</td>' +
                '</tr>'
        );
