$(function () {
    ...
    reports.ajax_cross = {
        get_control_bp: function (year,url, cb) {
            var url = url+'ckd_02_02_06_original.php',
                params = {
                    year: year
                }
    ...
    reports.set_control_bp = function (data) {
        $('#tbl_list > tbody').append(
            '<tr>' +
                ...
                '<td>' + app.add_commars_with_out_decimal(data.ckd_ldl) + '</td>' +
                '<td>' + app.add_commars_with_out_decimal(data.ckd_ldl_ok) + '</td>' +
                '<td>' + app.add_commars_with_out_decimal(data.ckd_ldl_not_ok) + '</td>' +
                '</tr>'
        );
