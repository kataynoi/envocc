$(document).ready(function(){
    ...
    var thaimed = {};
    thaimed.ajax = {
        get_drugthai: function (items, cb) {
            var url = '/thaimed/get_drugthai',
                params = {
                    items: items
                }
    ...
    thaimed.get_drugthai = function(items){
        $('#tbl_ncd_list > tbody').empty();
        thaimed.ajax.get_drugthai(items, function (err, data) {
    ...
    $(document).on('click', 'button[data-name="btn_show_drugthai"]', function(e) {
        e.preventDefault();
