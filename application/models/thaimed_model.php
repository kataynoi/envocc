...
class Thaimed_model extends CI_Model
{
    public $hospcode;
    public $year;
    ...
    public function get_drugthai($year)
    {
        ...
        $sql .= " FROM (SELECT * FROM drug_opd WHERE DATE_FORMAT(date_serv,'%Y%m') BETWEEN '".($year-1)."10' AND '".($year)."09' AND didstd LIKE '4%' GROUP BY CONCAT(HOSPCODE,seq)) as a";
    ...
    public function get_drugthai_by_amp($year,$amp)
    {
    ...
    public function get_drugthai_by_hospcode($year,$hospcode)
    {
        $sql = "SELECT a.didstd as id,a.dname as name ";
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m')='".($year-1)."10',1,0)) as m10";
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m')='".($year-1)."11',1,0)) as m11";
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m')='".($year-1)."12',1,0)) as m12";
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m')='".($year)."01',1,0)) as m01";
        ...
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m')='".($year)."09',1,0)) as m09";
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m') BETWEEN '".($year-1)."10' AND '".($year)."09',1,0)) as total";
        $sql .= " FROM (SELECT * FROM drug_opd WHERE HOSPCODE='".$hospcode."' AND DATE_FORMAT(date_serv,'%Y%m') BETWEEN '".($year-1)."10' AND '".($year)."09' AND didstd LIKE '4%' GROUP BY CONCAT(HOSPCODE,seq)) as a";
        /*$sql .=" LEFT JOIN ex_co_office b ON  a.HOSPCODE=b.off_id";
        $sql .=" LEFT JOIN ex_co_district c ON b.distid=c.distid";*/
        $sql.=" WHERE a.HOSPCODE='".$hospcode."' ";
        $sql .=" GROUP BY a.didstd ORDER BY total DESC";
 ...
 //##############  หัตถการแพทย์แผนไทย
    public function get_procedure($year)
    {
        ...
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m')='".($year)."09',1,0)) as m09";
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m') BETWEEN '".($year-1)."10' AND '".($year)."09',1,0)) as total";
        $sql .= " FROM (SELECT * FROM procedure_opd WHERE DATE_FORMAT(date_serv,'%Y-%m') BETWEEN '".($year-1)."-10' AND '".($year)."-09' AND PROCEDCODE in( SELECT procedcode FROM icd10tmttm) ) as a";
        $sql .=" LEFT JOIN ex_co_office b ON  a.HOSPCODE=b.off_id";
        $sql .=" LEFT JOIN ex_co_district c ON b.distid=c.distid";
        $sql .=" GROUP BY b.distid";
        $rs = $this->hdc->query($sql)->result();
    ...
    {
        $sql = "SELECT a.PROCEDCODE as id,c.proceddesc as name ";
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m')='".($year-1)."10',1,0)) as m10";
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m')='".($year-1)."11',1,0)) as m11";
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m')='".($year-1)."12',1,0)) as m12";
        $sql .= ",SUM(IF(DATE_FORMAT(a.date_serv,'%Y%m')='".($year)."01',1,0)) as m01";
        ...
        $sql .= " FROM (SELECT * FROM procedure_opd WHERE DATE_FORMAT(date_serv,'%Y%m') BETWEEN '".($year-1)."10' AND '".($year)."09' AND PROCEDCODE in( SELECT procedcode FROM icd10tmttm) ) as a";
        $sql .=" LEFT JOIN ex_co_office b ON  a.HOSPCODE=b.off_id";
        $sql .=" LEFT JOIN icd10tmttm c ON a.PROCEDCODE=c.procedcode";
        $sql.=" WHERE a.HOSPCODE='".$hospcode."' ";
